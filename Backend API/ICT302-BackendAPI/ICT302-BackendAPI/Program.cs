...
builder.Services.AddCors(options =>
{
    options.AddPolicy(cors, policy =>
    {
        policy.WithOrigins("http://localhost:3000", "https://localhost:3000", "http://10.51.33.50",
                ...
                "https://www.wildvision.co", "https://www.api.wildvision.co")
            .AllowAnyMethod()
...
{
    if (!int.TryParse(builder.Configuration["JobQueueCapacity"], out var queueCapacity)) queueCapacity = 10;
...
var context = app.Services.GetRequiredService<SchemaContext>();
app.Logger.LogInformation("Checking if database is available...");
if (await context.CheckDbIsAvailable())
{
    var monitorJobLoop = app.Services.GetRequiredService<MonitorJobLoop>()!;
