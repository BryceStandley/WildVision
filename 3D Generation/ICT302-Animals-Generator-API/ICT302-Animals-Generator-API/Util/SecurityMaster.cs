...
/**
 * <summary>Utility Singleton for auth requests to the Generator API</summary>
 */
public sealed class SecurityMaster(ILogger<SecurityMaster> logger, IConfiguration configuration)
{
    /**
     * <summary>The internal auth token that is loaded from the appsettings.json file at runtime</summary>
     * <remarks>This token must match the same token within the backend API appsettings for successful auth requests</remarks>
     */
    private readonly string? _internalAuthToken = configuration["AuthToken"] ?? "";

    /// <summary>Logger to log to the console and log file</summary>
    private readonly ILogger<SecurityMaster> _logger = logger;
